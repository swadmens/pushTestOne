...
+(void)createCustomUIWithBlack:(BOOL)isBlack Report:(BOOL)isReport Delete:(BOOL)isDelete
{
    ...
    //分享图标和标题数组
    NSArray *imageNameArr = @[@"share_qqzone_image",@"share_wechat_frined_image",@"share_wechat_circle_image",@"share_weibo_image"];
    NSMutableArray *imagesArr = [NSMutableArray array];
    for (NSString *imageName in imageNameArr) {
        UIImage *image = [UIImage imageNamed:imageName];
        [imagesArr addObject:image];
    }
    NSArray *titlesArr = @[@"QQ",@"微信",@"朋友圈",@"微博"];
    ...
    if (isReport) {
        [imagesArr addObject:[UIImage imageNamed:@"detail_jubao_images"]];
